...
pub fn write_serde_default(w: &mut CodeWriter, customize: &Customize) {
    if customize.serde_derive.unwrap_or(false) && customize.serde_derive_default.unwrap_or(false) {

        w.write_line(&format!("#[cfg_attr(feature = \"with-serde\", {})]", "serde(default)"));
